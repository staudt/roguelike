...
const DEAD_END_CHANCE = 0.25;  // chance per corridor tile to spawn a branch
const DEAD_END_MIN = 2;
const DEAD_END_MAX = 6;
const MAX_DEAD_ENDS = 4;       // cap per dungeon so they stay uncommon
...
function addDeadEnds(tiles: TileMap): void {
  ...
  // Shuffle and try to place branches
  ...
  for (const [cx, cy] of corridors) {
    ...
    // Check the whole path is carve-able (all walls, within bounds with margin)
    let valid = true;
    for (let s = 1; s <= length; s++) {
      ...
      // Make sure we don't carve adjacent to a room or another corridor (except our own path)
      for (const [cdx, cdy] of DIRS) {
        const ax = nx + cdx;
        const ay = ny + cdy;
        if (ax === nx - ddx && ay === ny - ddy) continue; // skip the tile behind us
...
export function generateDungeon(): DungeonResult {
  const tiles = createTileMap(MAP_WIDTH, MAP_HEIGHT);

  const root: BSPNode = { x: 0, y: 0, w: MAP_WIDTH, h: MAP_HEIGHT };
  splitBSP(root);
  placeRooms(root);
  ...
  // Occasionally add dead-end branches off corridors
  addDeadEnds(tiles);

  // Pick start and stairs rooms (farthest apart)
